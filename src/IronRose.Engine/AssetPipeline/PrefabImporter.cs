...
//          TOML 파싱은 TomlConfig.LoadFile()을 사용한다.
// @deps    TomlConfig, AssetDatabase, SceneSerializer, PrefabUtility, RoseEngine/EditorDebug
// @exports
...
//          LoadBase()에서 config.ToTomlString()으로 재직렬화하여 SceneSerializer에 전달 (문자열 기반 API).
...
namespace IronRose.AssetPipeline
{
    ...
    public class PrefabImporter
    {
        ...
        private GameObject? LoadVariant(TomlConfig config, string basePrefabGuid, string variantPath, int depth)
        {
            ...
            // 3. 오버라이드 적용 -- GetRawTable() 사용 (SceneSerializer.ApplyOverrides가 TomlTableArray를 받으므로)
